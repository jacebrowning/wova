...
def as_completed_buffered(future_iterable, buffer_size=None):
    """
    Purpose:
        Stream an iterable of futures over an executor with a constant buffer size.
    
        Creating futures consumes all of an input iterator before work starts. That
        is a problem when the data set being processed is large because it pulls the
        entire set into memory
    ...
    buf = deque(x for _, x in zip(b_rng, future_iterable))
